...
export interface PluginInfo<PLUGINPARAMS, TARGET extends IGeneralMetadata<HookMap>, SYMB extends Plugin<PLUGINPARAMS, TARGET>> {
    key: SYMB;
    props: PLUGINPARAMS;
}
...
export declare const createPlugin: <TARGET extends IGeneralMetadata<HookMap> = IGeneralMetadata<HookMap<never>>>() => <PluginProps>(pluginName: string, defaultProps: Partial<PluginProps>, plugin: ReplaceParams<NonNullable<TARGET["__hooks"]>, PluginProps>, merge?: (params: PluginProps[]) => PluginProps[]) => Plugin<PluginProps, TARGET>;
...
export interface IRenderableMetadataBase<HOOKS extends HookMap = HookMap> extends IGeneralMetadata<HOOKS & IRenderableLifeCycleHooks> {
    /**
